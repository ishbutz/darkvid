...
(() => {
    ...
    // Constants
    const iconUrl = chrome.runtime.getURL('icons/darkvid.png');
    const delay = 3000;
    ...
    document.addEventListener('fullscreenchange', e => {
        ...
        let panel = parent.querySelector('.darkvid-panel-video');
        if (!video || !video.hasAttribute('darkvid-id')) return;
        let id = video.getAttribute('darkvid-id');
        if (panel) {
            if (!document.fullscreenElement) document.body.appendChild(panel);
            setPanelPos(panel, video);
            return;
        }
        panel = document.querySelector(`.darkvid-panel-video[darkvid-id="${id}"]`);
        if (panel) {
            setPanelPos(panel, video);
    ...
    function checkMedias(ms) {
        ...
        document.querySelectorAll(ms).forEach(media => {
            if (media.hasAttribute('darkvid-id') || isHidden(media)) return;
            let panel = createPanel(media, counter);
            panel.setAttribute('darkvid-id', counter);
            media.setAttribute('darkvid-id', counter);
            setPanelPos(panel, media);
    ...
    function createPanel(media, cntid) {
        ...
        button.className = 'darkvid-button';
        button.innerHTML = `<img src="${iconUrl}">`;
        button.addEventListener('click', e => {
            if (dropdown.classList.contains('darkvid-dropdown-show')) {
                button.innerHTML = `<img src="${iconUrl}">`;
                dropdown.classList.remove('darkvid-dropdown-show');
            } else {
                button.innerHTML = '&#10006;';
                dropdown.classList.add('darkvid-dropdown-show');
            }
        });
        ...
        reset.addEventListener('click', e => {
            ...
            dropdown.querySelectorAll('input').forEach(input => {
                if (input.type === 'checkbox') {
                    ...
                    let key = input.getAttribute('darkvid-key');
        ...
        // Panel
        panel.lang = 'en';
        panel.className = `darkvid-panel darkvid-panel-${ms}`;
        dropdown.className = 'darkvid-dropdown';
        dpc.className = 'darkvid-dpc';
    ...
    function createField(min, max, step, key, data, media) {
        ...
        // Label
        label.innerText = key;
        label.className = 'darkvid-label';
        field.appendChild(label);


        // Range input
        range.setAttribute('type', 'range');
        range.className = 'darkvid-range';
        range.min = min;
        range.max = max;
        range.step = step;
        range.value = data[key];
        range.setAttribute('darkvid-key', key);
        ...
        number.className = 'darkvid-number';
        number.lang = 'en';
        number.min = min;
        number.max = max;
        number.step = step;
        ...
        number.setAttribute('darkvid-key', key);
        number.addEventListener('input', onChange);
        field.appendChild(number);


        field.className = 'darkvid-field';
        return field;
    }
    ...
    function createCheckbox(name, title, change) {
        ...
        let label = document.createElement('label');
        div.className = 'darkvid-checkbox';
    ...
    function offset(el) {
        ...
        let offset = {};
        if (el.hasAttribute('darkvid-transformed') && el.offsetParent) {
    ...
    function updateMedia(media, data) {
        if (media.tagName === 'VIDEO') {
            ...
            media.style.transform = `scale(${data.fh * data.scale}, ${data.fv * data.scale}) rotate(${data.rotate}deg)`;
            if (data.rotate || data.scale != 1) media.setAttribute('darkvid-transformed', '');
            else media.removeAttribute('darkvid-transformed');
            if (data.controls) media.setAttribute('controls', '');
            else media.removeAttribute('controls');
        }
